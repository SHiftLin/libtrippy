...
use backend::{trace::Trace, Backend};
use config::{LogFormat, LogSpanEvents, Mode, TrippyConfig};
use dns::{Config, DnsResolver, Resolver};
...
use tracing::{
    MultipathStrategy, PortDirection, PrivilegeMode, SourceAddr, TracerAddrFamily,
...
pub fn start_dns_resolver(cfg: &TrippyConfig) -> anyhow::Result<DnsResolver> {
...
pub fn create_geoip_lookup(cfg: &TrippyConfig) -> anyhow::Result<GeoIpLookup> {
...
pub fn configure_logging(cfg: &TrippyConfig) -> Option<FlushGuard> {
    if cfg.verbose {
